...
export function CustomerProposals() {
  ...
  const handleAcceptProposal = async (proposalId: string) => {
    ...
    const newServiceOrder = {
      id: `OS-${Math.floor(Math.random() * 1000)}`,
      type: acceptedProposal.title,
      status: 'pending',
      description: acceptedProposal.description,
      scheduledFor: new Date(),
      technician: acceptedProposal.technician,
      location: 'Endereço informado pelo cliente',
      checklist: acceptedProposal.items.map((item) => ({
        item: item.description,
        completed: false,
      })),
      photos: [],
      notes: 'Ordem gerada a partir de orçamento aceito',
      documents: [],
    };

    try {
      const mod = await import('@/lib/mockServiceOrders');
      mod.mockServiceOrders.push(newServiceOrder);
      alert('Ordem de Serviço gerada com sucesso!');
    } catch (error) {
      console.error('Erro ao gerar ordem de serviço:', error);
      alert('Falha ao gerar ordem de serviço.');
    }
  };
